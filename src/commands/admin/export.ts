	...
	async execute(interaction: CommandInteraction, client: EClient) {
		...
		for (const [user, data] of Object.entries(allUser)) {
			...
			for (const char of chara) {
				const stats = await getUserFromMessage(client.settings, user, interaction, char.charName, { skipNotFound: true, fetchAvatar: true });
				if (!stats) continue;
		...
		const columns = ["user", "charName", "avatar"];
